...
png.fromCompressedData = function(width, height, compressedData) {
  return png.fromEmpty(width, height).then(template => {
    ...
    let totalBytes = png.headerBytes + png.chunkHeaderBytes +
                     chunkLength + png.chunkFooterBytes + png.footerBytes;
    let pngData = new ArrayBuffer(totalBytes);
    let dataView = new DataView(pngData);
    buffer.copy(template, 0, pngData, 0, png.headerBytes);
    dataView.setUint32(png.headerBytes, chunkLength);
    dataView.setUint32(png.headerBytes + 4, png.idatTag);
    buffer.copy(compressedData, 0,
                pngData, png.headerBytes + png.chunkHeaderBytes,
                compressedData.byteLength);
    buffer.copy(template, template.byteLength - png.footerBytes,
                pngData, totalBytes - png.footerBytes, png.footerBytes);
    return pngData;
  });
}
...
// Extracts compressed data from the PNG file in the provided ArrayBuffer.
//
// Returns an ArrayBuffer with zlib compressed image data followed by a CRC-32.
png.getCompressedData = function(pngData) {
  let dataView = new DataView(pngData);
  if (dataView.getUint32(png.headerBytes + 4) != png.idatTag) {
    throw new Error('Expected IDAT chunk after the header');
  }
  let chunkOffset = png.headerBytes + png.chunkHeaderBytes;
  let chunkLength = dataView.getUint32(png.headerBytes);
  // Include the chunk footer because CRC is difficult to recompute in JS.
  return pngData.slice(chunkOffset,
                       chunkOffset + chunkLength + png.chunkFooterBytes);
}
